...
namespace Chat
{
	int ClientConnection::connectToServer() 
	{
        ...
        errstate = listen(con.getSock(), SOMAXCONN); // code don't interrupt
        if (errstate != 0)
        {
            std::cout << "Error in binding ip:port to socket #" << WSAGetLastError() << std::endl;
            return closeOnError(con.getPSock());
        }

        sockaddr_in clientInfo;
        int clientInfoSize = sizeof(clientInfo);

        SOCKET clientConn = accept(Sock, (sockaddr*)&clientInfo, &clientInfoSize); // interuption. socket from client
